...
unsigned long	flashBase, tfsOffset;
...
main(int argc,char *argv[])
{
	...
	uchar	*flash, *end;
	char	*filename, *command, *outfile, *space;
	...
	flashBase = 0;
	tfsOffset = 0;
	while((opt=getopt(argc,argv,"b:dehmo:V")) != EOF) {
		switch(opt) {
		case 'b':
	...
	if (strcmp(command,"ls") == 0) {
		...
		while(1) {
			...
			hdrstart = flashBase + tfsOffset + ((char *)this - (char *)flash);
			printf("\nFile%3d:\n Hdr at: 0x%08x %s %s\n",++ftot,hdrstart,
		...
		if (!hdrcrcfail) {
			...
			postTFSTest(flash,(uchar *)this,mstat.st_size-tfsOffset);
		}
	}
		...
		while(1) {
			...
			if (debug) {
				fprintf(stderr,"File @ 0x%lx: %s\n",
					flashBase + tfsOffset + ((char *)this - (char *)flash),
					fhdr.name);
			}
			if ((flashBase + tfsOffset + ((char *)this - (char *)flash)) == offset) {
...
char	*usage_txt[] = {
	"Usage: tfs [b:ehmo:V] {infile} {command} [command args]",
	"Options:",
	...
	"   -o {hex_addr}   offset of start of TFS from base of file",
