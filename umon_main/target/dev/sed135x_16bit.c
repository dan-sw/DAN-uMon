...
ulong sed135x_init()
{
	...
	// THIS LOOP TAKES AN EXCESSIVE AMOUNT OF TIME, so if possible,
	// this function should be called AFTER iCache is enabled.
...
sed_scroll()
{
	...
	if (sed_row > (ROWS_PER_SCREEN - 1))
	{
		...
		// go from active to non-active, unless the display is not enabled
			...
			SED1356_REG_CRT_DISP_START_HI  = H2SED(((sed_fb_offset & 0x030000) >> 16));
		}
		else // LCD
		{
			...
			SED1356_REG_LCD_DISP_START_HI  = H2SED(((sed_fb_offset & 0x030000) >> 16));
		}
	} // if (sed_row > (ROWS_PER_SCREEN - 1))
	return;
		...
		switch (sed_color_depth){
			...
			case 16:
				...
				for (font_row = 0; font_row < FONT_HEIGHT; font_row++){
					...
					{
						// get a words worth of pixels
						wr16 = ((font_data8 & 0x80) ? vga_lookup[sed_fg_color] : vga_lookup[sed_bg_color]);
...
//
char *sedHelp[] = {
	"Set S1D13506 video controller options",
    "-[c,l,f,b,d:[4,8,16]]",
	"",
    " Options...",
	...
	"",
	" This command allows the user to set options for the S1D13506",
	" video controller on this board.  The user may set the mode to",
	" CRT or LCD, set the color depth to run at, and set the foreground",
	" and background colors.",
	" Note that all but the foreground and background options will cause",
	" the S1D1350x to be re-initialized.",
	0
};
