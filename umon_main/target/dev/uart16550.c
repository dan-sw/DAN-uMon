...
#define CONSOLE(offset) *(volatile unsigned*)(CONSOLE_UART_BASE+offset)
...
void ConsoleTxFlush()
{
	uint32 reg;
	do{
		reg = CONSOLE(UART_USR_REG);

	}while(!((reg & UART_USR_TFE) == UART_USR_TFE) || ((reg & UART_USR_BUSY) == UART_USR_BUSY));
}
...
ResetUART(void)
{
	...
	// Reset UART
	CONSOLE(UART_SRR_REG)  = UART_SRR_UR;
	for(i=0;i<0xfff;i++);
	CONSOLE(UART_SRR_REG) = ~UART_SRR_UR;
}
