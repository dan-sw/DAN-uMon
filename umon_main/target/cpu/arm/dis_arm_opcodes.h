...
#ifndef DIS_ARM_OPCODES_H
#define DIS_ARM_OPCODES_H

/**************************************************************************/

struct arm_opcode {
    unsigned long value, mask; /* recognise instr if (op&mask)==value */
    char *assembler; /* how to disassemble this instruction */
};

struct thumb_opcode
{
    unsigned short value, mask;	/* recognise instr if (op&mask)==value */
    char * assembler; /* how to disassemble this instruction */
};
...
Thumb specific format options:
   %D            print Thumb reg (bits 0..2 as high number if bit 7 set)
   %S            print Thumb reg (bits 3..5 as high number if bit 6 set)
...
/* Note: There is a partial ordering in this table. 
   It must be searched from the top to obtain a correct match. */
...
{
  /* Thumb instructions */
  {0x46C0, 0xFFFF, "nop\t(mov r8,r8)"}, /* format 5 instrs do not update the PSR */
