...
/* DAN: New function to support 4-byte aligned memory access.
		Rewritten based on unZip().
*/
...
int decompress_4aligned(char *src, int srclen, char *dest)
{
	int			len, dstlen;
	gz_stream	*s;
	char		*desttmp, *desttmp2, *dest2 = dest;
	enum { DSTTMP_SIZE = 1024 };
	...
	desttmp = desttmp2 = malloc(DSTTMP_SIZE + 8);
	if (!desttmp) {
		printf("decompress2(): malloc(%d) allocation failed\n", DSTTMP_SIZE);
		return CMD_FAILURE;
	}

	desttmp = (char*) ALIGN4BYTE_UP(desttmp);
	dstlen  = 0;

	for (;;)
	{
		len = gzRead(s,desttmp,DSTTMP_SIZE);
		if (len < 0) {
			printf("gzRead() returned error %d\n",len);
			break;
		}
		if (len==0)
			break;
		if ((unsigned)dest & 3) {
			printf ("decompress_4aligned: destination alignment problem! Internal error");
			break;
		}
		memcpy_4aligned ((int*)dest, (int*)desttmp, ALIGN4BYTE_UP(len));
		dstlen += len;
		dest += len;
	}
	...
	if (dstlen > 0) {
		flushDcache(dest2,dstlen);
		invalidateIcache(dest2,dstlen);
	}
