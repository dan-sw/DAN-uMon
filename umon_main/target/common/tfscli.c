 ...
 */
static int
tfsld(char *name,int verbose,char *sname,int verifyonly, int align4)
{
	int		err;
	TFILE	*fp;
	...
	if (fp->flags & TFS_EBIN) {
		...
		err = tfsloadebin(fp,verbose,&entry,sname,verifyonly, align4);
		if (err == TFS_OKAY) {
			char buf[16];
...
Tfs(int argc, char *argv[])
{
	...
	TINFO	tinfo;
	long	size;
	int		err, opt, verbose, i, more, retval, status, align4;
	...
	devprefix = (char *)0;
    align4 = 0;
	while ((opt = getopt(argc, argv, "d:vmf:i:4")) != -1) {
		switch (opt) {
		case 'd':
			devprefix = optarg;
	 ...
	 */
		...
		if (arg1[2] == 'v')
			...
			return(CMD_PARAM_ERROR);
		status = tfsld(arg2,verbose,arg3,vfy,align4);
