...
//int main()
{
	...
	unsigned int input_buf[14336*1*10];
...
if((argc == 2))
{
   ...
   10Mhz\\Ant0,10Mhz\\Ant1,20Mhz\\Ant0 and 20Mhz\\Ant1 folders\n\n");
...
printf("--------------------------------------------------------------------------------\n");
    ...
    for(j=0;j<10;j++)
    {
        ...
        if (fpin[j] == NULL)
        {
        	printf(">>> Error: Input file %s is missing; verify all needed files exists and re-run\n",str_in);
        	return(1);
        }
    ...
    if (fpout == NULL)
    {
    	printf(">>> Error: Output directory 'outputBinary' is missing; create and re-run\n",str_in);
    	return(1);
    }
